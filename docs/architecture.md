...
- **Demo contact seeding** — On `initSession()`, after loading saved contacts, `seedDemoContact()` adds a single contact (configured via `DEMO_CONTACT_NAME` + `DEMO_CONTACT_PHONE`) the first time it runs for a uid. The seed is gated by a `meta.demoSeeded` flag inside `sessions/{uid}/saved-contacts.json`, so deleting the demo contact does not cause it to be re-added on restart. This lets new users test "send a WhatsApp to {name}" without first saving a real contact.
